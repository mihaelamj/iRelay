...
The SKILL.md format (YAML frontmatter + markdown body) is the contract. iRelay needs to parse this format, evaluate the gating rules (OS, bins, env, config), manage the prompt budget (count + character limits), and handle env var injection with the same safety guarantees.
